...
export class ConversationController {
  ...
  /**
   * Time-of-day greeting for the empty-chat welcome screen.
   *
   * Deliberately impersonal: the user's identity lives in the Octo profile
   * (`user.md`), which the server does not expose over its HTTP API, and the
   * server may not even be on this machine. Guessing a name plugin-side would
   * shadow a provider-owned concern.
   */
  getGreeting(): string {
    ...
    if (hour >= 18 && hour < 22) return t('chat.greeting.evening');
