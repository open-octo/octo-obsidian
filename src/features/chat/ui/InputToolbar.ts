...
export class ModeSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    ...
    this.container = parentEl.createDiv({ cls: 'octo-mode-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    this.labelEl = this.container.createSpan({ cls: 'octo-mode-label' });
    this.toggleEl = this.container.createDiv({ cls: 'octo-toggle-switch' });
  ...
  updateDisplay() {
    ...
    if (!selectorConfig || selectorConfig.options.length !== 2) {
      this.container.addClass('octo-hidden');
      return;
    }

    this.container.removeClass('octo-hidden');
...
export class ThinkingBudgetSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'octo-thinking-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    this.effortEl = this.container.createDiv({ cls: 'octo-thinking-effort' });
    const effortLabel = this.effortEl.createSpan({ cls: 'octo-thinking-label-text' });
    effortLabel.setText('Effort:');
    this.effortGearsEl = this.effortEl.createDiv({ cls: 'octo-thinking-gears' });

    // Legacy budget selector (for custom models)
    this.budgetEl = this.container.createDiv({ cls: 'octo-thinking-budget' });
    const budgetLabel = this.budgetEl.createSpan({ cls: 'octo-thinking-label-text' });
    budgetLabel.setText('Thinking:');
    this.budgetGearsEl = this.budgetEl.createDiv({ cls: 'octo-thinking-gears' });
  ...
  private renderEffortGears() {
    ...
    const currentEl = this.effortGearsEl.createDiv({ cls: 'octo-thinking-current' });
    currentEl.setText(currentInfo?.label || options[0]?.label || 'High');

    const optionsEl = this.effortGearsEl.createDiv({ cls: 'octo-thinking-options' });

    for (const effort of [...options].reverse()) {
      const gearEl = optionsEl.createDiv({ cls: 'octo-thinking-gear' });
  ...
  private renderBudgetGears() {
    ...
    const currentEl = this.budgetGearsEl.createDiv({ cls: 'octo-thinking-current' });
    currentEl.setText(currentBudgetInfo?.label || options[0]?.label || 'Off');

    const optionsEl = this.budgetGearsEl.createDiv({ cls: 'octo-thinking-options' });
  ...
  updateDisplay() {
    ...
    if (capabilities.reasoningControl === 'none') {
      this.effortEl?.addClass('octo-hidden');
      this.budgetEl?.addClass('octo-hidden');
      return;
    }
    ...
    if (this.effortEl) {
      this.effortEl.toggleClass('octo-hidden', !adaptive);
    }
    if (this.budgetEl) {
      this.budgetEl.toggleClass('octo-hidden', adaptive);
    }
...
export class PermissionToggle {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'octo-permission-toggle' });
    this.render();
  }
  ...
  private render() {
    ...
    this.labelEl = this.container.createSpan({ cls: 'octo-permission-label' });
    this.toggleEl = this.container.createDiv({ cls: 'octo-toggle-switch' });
  ...
  updateDisplay() {
    ...
    if (canShowPlan && planValue && mode === planValue) {
      this.toggleEl.addClass('octo-hidden');
      this.labelEl.setText(planLabel);
      this.labelEl.addClass('plan-active');
    } else {
      this.toggleEl.removeClass('octo-hidden');
...
export class ServiceTierToggle {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'octo-service-tier-toggle' });
    this.render();
  }
  ...
  private render() {
    ...
    this.buttonEl = this.container.createDiv({ cls: 'octo-service-tier-button' });
    this.iconEl = this.buttonEl.createSpan({ cls: 'octo-service-tier-icon' });
...
export class ExternalContextSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'octo-external-context-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    const iconWrapper = this.container.createDiv({ cls: 'octo-external-context-icon-wrapper' });

    this.iconEl = iconWrapper.createDiv({ cls: 'octo-external-context-icon' });
    setIcon(this.iconEl, 'folder');

    this.badgeEl = iconWrapper.createDiv({ cls: 'octo-external-context-badge' });
    ...
    this.dropdownEl = this.container.createDiv({ cls: 'octo-external-context-dropdown' });
    this.renderDropdown();
  }
  ...
  private renderDropdown() {
    ...
    // Header
    const headerEl = this.dropdownEl.createDiv({ cls: 'octo-external-context-header' });
    headerEl.setText('External contexts');

    // Path list
    const listEl = this.dropdownEl.createDiv({ cls: 'octo-external-context-list' });

    if (this.externalContextPaths.length === 0) {
      const emptyEl = listEl.createDiv({ cls: 'octo-external-context-empty' });
      emptyEl.setText('Click folder icon to add');
    } else {
      for (const pathStr of this.externalContextPaths) {
        const itemEl = listEl.createDiv({ cls: 'octo-external-context-item' });

        const pathTextEl = itemEl.createSpan({ cls: 'octo-external-context-text' });
        ...
        const lockBtn = itemEl.createSpan({ cls: 'octo-external-context-lock' });
        if (isPersistent) {
        ...
        const removeBtn = itemEl.createSpan({ cls: 'octo-external-context-remove' });
        setIcon(removeBtn, 'x');
...
export class McpServerSelector {
  ...
  constructor(parentEl: HTMLElement) {
    this.container = parentEl.createDiv({ cls: 'octo-mcp-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    const iconWrapper = this.container.createDiv({ cls: 'octo-mcp-selector-icon-wrapper' });

    this.iconEl = iconWrapper.createDiv({ cls: 'octo-mcp-selector-icon' });
    appendMcpIcon(this.iconEl);

    this.badgeEl = iconWrapper.createDiv({ cls: 'octo-mcp-selector-badge' });

    this.updateDisplay();

    this.dropdownEl = this.container.createDiv({ cls: 'octo-mcp-selector-dropdown' });
    this.renderDropdown();
  ...
  private renderDropdown() {
    ...
    // Header
    const headerEl = this.dropdownEl.createDiv({ cls: 'octo-mcp-selector-header' });
    headerEl.setText('Mcp servers');

    // Server list
    const listEl = this.dropdownEl.createDiv({ cls: 'octo-mcp-selector-list' });

    const allServers = this.mcpManager?.getServers() || [];
    const servers = allServers.filter(s => s.enabled);

    if (servers.length === 0) {
      const emptyEl = listEl.createDiv({ cls: 'octo-mcp-selector-empty' });
  ...
  private renderServerItem(listEl: HTMLElement, server: ManagedMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'octo-mcp-selector-item' });
    ...
    // Checkbox
    const checkEl = itemEl.createDiv({ cls: 'octo-mcp-selector-check' });
    if (isEnabled) {
      appendCheckIcon(checkEl);
    }

    // Info
    const infoEl = itemEl.createDiv({ cls: 'octo-mcp-selector-item-info' });

    const nameEl = infoEl.createSpan({ cls: 'octo-mcp-selector-item-name' });
    nameEl.setText(server.name);

    // Badges
    if (server.contextSaving) {
      const csEl = infoEl.createSpan({ cls: 'octo-mcp-selector-cs-badge' });
      csEl.setText('@');
  ...
  private toggleServer(name: string, itemEl: HTMLElement) {
    ...
    const checkEl = itemEl.querySelector<HTMLElement>('.octo-mcp-selector-check');
...
export class ContextUsageMeter {
  ...
  constructor(parentEl: HTMLElement) {
    this.container = parentEl.createDiv({ cls: 'octo-context-meter' });
    this.render();
    // Initially hidden
    this.container.addClass('octo-hidden');
  }
  ...
  setVisible(visible: boolean): void {
    this.container.toggleClass('octo-hidden', !visible);
  }
  ...
  private render() {
    ...
    const gaugeEl = this.container.createDiv({ cls: 'octo-context-meter-gauge' });
    ...
    backgroundPath.classList.add('octo-meter-bg');
    ...
    fillPath.classList.add('octo-meter-fill');
    ...
    this.percentEl = this.container.createSpan({ cls: 'octo-context-meter-percent' });
  }
