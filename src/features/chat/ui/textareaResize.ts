...
export function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  const viewHeight = textarea.closest('.octo-container')?.clientHeight ?? window.innerHeight;
  const maxHeight = calculateTextareaMaxHeight(viewHeight);

  textarea.setCssProps({
    '--octo-textarea-min-height': `${TEXTAREA_BASE_MIN_HEIGHT}px`,
    '--octo-textarea-max-height': `${maxHeight}px`,
  });
  ...
  textarea.setCssProps({
    '--octo-textarea-min-height': `${minHeight}px`,
    '--octo-textarea-max-height': `${maxHeight}px`,
  });
}
