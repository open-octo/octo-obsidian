...
export class NavigationSidebar {
  ...
  constructor(
    ...
    this.container = this.parentEl.createDiv({ cls: 'octo-nav-sidebar' });

    // Create buttons
    this.topBtn = this.createButton('octo-nav-btn-top', 'chevrons-up', 'Scroll to top');
    this.prevBtn = this.createButton('octo-nav-btn-prev', 'chevron-up', 'Previous message');
    this.tocBtn = this.createButton('octo-nav-btn-toc', 'list-tree', 'Conversation directory');
    this.nextBtn = this.createButton('octo-nav-btn-next', 'chevron-down', 'Next message');
    this.bottomBtn = this.createButton('octo-nav-btn-bottom', 'chevrons-down', 'Scroll to bottom');
  ...
  private createButton(cls: string, icon: string, label: string): HTMLElement {
    const btn = this.container.createDiv({ cls: `octo-nav-btn ${cls}` });
    setIcon(btn, icon);
  ...
  private applyVisibility(): void {
    ...
    this.tocBtn.classList.remove('octo-hidden');
  ...
  private getDirectoryEntries(): Array<{ el: HTMLElement; title: string }> {
    return Array.from(this.messagesEl.querySelectorAll<HTMLElement>('.octo-message-user, [data-role="user"]'))
      .map(el => ({
        el,
  ...
  private getDirectoryTitle(el: HTMLElement): string {
    ...
    const contentEl = el.querySelector<HTMLElement>('.octo-message-content');
  ...
  private nodeContainsDirectoryMessage(node: Node): boolean {
    ...
    return typeof candidate.querySelector === 'function'
      && candidate.querySelector('.octo-message-user, [data-role="user"]') !== null;
  }
  ...
  private isDirectoryMessageElement(node: Node): boolean {
    ...
    if (typeof candidate.matches === 'function') {
      return candidate.matches('.octo-message-user, [data-role="user"]');
    }
    return candidate.classList?.contains?.('octo-message-user') === true
  ...
  private openDirectory(): void {
    ...
    this.closeDirectory();
    this.tocPopover = this.parentEl.createDiv({ cls: 'octo-nav-toc-popover' });
    this.tocPopover.createDiv({ cls: 'octo-nav-toc-title', text: 'Conversation directory' });
    const listEl = this.tocPopover.createDiv({ cls: 'octo-nav-toc-list' });

    if (entries.length === 0) {
      listEl.createDiv({
        cls: 'octo-nav-toc-empty',
  ...
  private scrollToMessage(direction: 'prev' | 'next'): void {
    const messages = Array.from(this.messagesEl.querySelectorAll<HTMLElement>('.octo-message-user'));
