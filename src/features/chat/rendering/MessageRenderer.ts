...
export class MessageRenderer {
  ...
  addMessage(msg: ChatMessage): HTMLElement {
    ...
    const msgEl = this.messagesEl.createDiv({
      cls: `octo-message octo-message-${msg.role}`,
      attr: {
    ...
    const contentEl = msgEl.createDiv({ cls: 'octo-message-content', attr: { dir: 'auto' } });

    if (msg.role === 'user') {
      const textToShow = this.getUserMessageTextToShow(msg);
      if (textToShow) {
        const textEl = contentEl.createDiv({ cls: 'octo-text-block' });
  ...
  updateLiveUserMessage(msg: ChatMessage): void {
    ...
    const toolbar = msgEl.querySelector<HTMLElement>('.octo-user-msg-actions');
    if (toolbar) {
      toolbar.querySelectorAll('.octo-user-msg-copy-btn').forEach((el) => el.remove());
    }
  ...
  renderMessages(
    ...
    const newWelcomeEl = this.messagesEl.createDiv({ cls: 'octo-welcome' });
    newWelcomeEl.createDiv({ cls: 'octo-welcome-greeting', text: getGreeting() });
  ...
  private renderInterruptMessage(): void {
    const msgEl = this.messagesEl.createDiv({ cls: 'octo-message octo-message-assistant' });
    const contentEl = msgEl.createDiv({ cls: 'octo-message-content', attr: { dir: 'auto' } });
    this.appendInterruptIndicator(contentEl);
  }
  ...
  private appendInterruptIndicator(contentEl: HTMLElement): void {
    const textEl = contentEl.createDiv({ cls: 'octo-text-block' });
    textEl.createSpan({ cls: 'octo-interrupted', text: 'Interrupted' });
    textEl.appendText(' ');
    textEl.createSpan({
      cls: 'octo-interrupted-hint',
      text: '\u00B7 What should Octo do instead?',
    });
  }
  ...
  private renderAssistantContent(msg: ChatMessage, contentEl: HTMLElement): void {
    if (msg.contentBlocks && msg.contentBlocks.length > 0) {
      ...
      for (const block of msg.contentBlocks) {
        if (block.type === 'thinking') {
          ...
          const boundaryEl = contentEl.createDiv({ cls: 'octo-compact-boundary' });
          boundaryEl.createSpan({ cls: 'octo-compact-boundary-label', text: 'Conversation compacted' });
    ...
    if (msg.durationSeconds && msg.durationSeconds > 0 && !hasCompactBoundary) {
      ...
      const footerEl = contentEl.createDiv({ cls: 'octo-response-footer' });
      footerEl.createSpan({
        text: `* ${flavorWord} for ${formatDurationMmSs(msg.durationSeconds)}`,
        cls: 'octo-baked-duration',
      });
    }
  }
  ...
  renderMessageImages(containerEl: HTMLElement, images: ImageAttachment[]): void {
    const imagesEl = containerEl.createDiv({ cls: 'octo-message-images' });

    for (const image of images) {
      const imageWrapper = imagesEl.createDiv({ cls: 'octo-message-image' });
  ...
  async renderContent(
    ...
    try {
      ...
      el.querySelectorAll('pre').forEach((pre) => {
        ...
        if (pre.parentElement?.classList.contains('octo-code-wrapper')) return;

        // Create wrapper
        const wrapper = createEl('div', { cls: 'octo-code-wrapper' });
  ...
  addTextCopyButton(textEl: HTMLElement, markdown: string): void {
    const copyBtn = textEl.createSpan({ cls: 'octo-text-copy-btn' });
  ...
  refreshActionButtons(msg: ChatMessage, allMessages?: ChatMessage[], index?: number): void {
    ...
    if (canRewind && this.rewindCallback && !msgEl.querySelector('.octo-message-rewind-btn')) {
      this.addRewindButton(msgEl, msg.id);
    }
    if (canFork && this.forkCallback && !msgEl.querySelector('.octo-message-fork-btn')) {
      this.addForkButton(msgEl, msg.id);
    }
  ...
  private cleanupLiveMessageEl(
    ...
    const needsRewind = expectedActions.canRewind
      && this.rewindCallback
      && !msgEl.querySelector('.octo-message-rewind-btn');
    const needsFork = expectedActions.canFork
      && this.forkCallback
      && !msgEl.querySelector('.octo-message-fork-btn');
  ...
  private getOrCreateActionsToolbar(msgEl: HTMLElement): HTMLElement {
    const existing = msgEl.querySelector<HTMLElement>('.octo-user-msg-actions');
    if (existing) return existing;
    return msgEl.createDiv({ cls: 'octo-user-msg-actions' });
  }

  private addUserCopyButton(msgEl: HTMLElement, content: string): void {
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const copyBtn = toolbar.createSpan({ cls: 'octo-user-msg-copy-btn' });
    setIcon(copyBtn, 'copy');
  ...
  private addRewindButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'octo-message-rewind-btn' });
  ...
  private addForkButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'octo-message-fork-btn' });
    if (toolbar.firstChild !== btn) toolbar.insertBefore(btn, toolbar.firstChild);
