...
export class InlineExitPlanMode {
  ...
  render(): void {
    ...
    const titleEl = this.rootEl.createDiv({ cls: 'octo-plan-inline-title' });
    titleEl.setText('Plan complete');

    this.planContent = this.readPlanContent();
    if (this.planContent) {
      const contentEl = this.rootEl.createDiv({ cls: 'octo-plan-content-preview' });
      if (this.renderContent) {
        void this.renderContent(contentEl, this.planContent);
      } else {
        contentEl.createDiv({ cls: 'octo-plan-content-text', text: this.planContent });
      }
    } else if (this.planReadError) {
      this.rootEl.createDiv({
        cls: 'octo-plan-content-preview octo-plan-read-error',
    ...
    if (allowedPrompts && Array.isArray(allowedPrompts) && allowedPrompts.length > 0) {
      const permEl = this.rootEl.createDiv({ cls: 'octo-plan-permissions' });
      permEl.createDiv({ text: 'Requested permissions:', cls: 'octo-plan-permissions-label' });
      const listEl = permEl.createEl('ul', { cls: 'octo-plan-permissions-list' });
    ...
    const newSessionRow = actionsEl.createDiv({ cls: 'octo-ask-item' });
    newSessionRow.addClass('is-focused');
    newSessionRow.createSpan({ text: '\u203A', cls: 'octo-ask-cursor' });
    newSessionRow.createSpan({ text: '1. ', cls: 'octo-ask-item-num' });
    newSessionRow.createSpan({ text: 'Approve (new session)', cls: 'octo-ask-item-label' });
    ...
    const approveRow = actionsEl.createDiv({ cls: 'octo-ask-item' });
    approveRow.createSpan({ text: '\u00A0', cls: 'octo-ask-cursor' });
    approveRow.createSpan({ text: '2. ', cls: 'octo-ask-item-num' });
    approveRow.createSpan({ text: 'Approve (current session)', cls: 'octo-ask-item-label' });
    ...
    const feedbackRow = actionsEl.createDiv({ cls: 'octo-ask-item octo-ask-custom-item' });
    feedbackRow.createSpan({ text: '\u00A0', cls: 'octo-ask-cursor' });
    feedbackRow.createSpan({ text: '3. ', cls: 'octo-ask-item-num' });
