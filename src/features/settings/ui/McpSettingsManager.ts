...
export class McpSettingsManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'octo-mcp-header' });
    headerEl.createSpan({ text: 'MCP Servers', cls: 'octo-mcp-label' });

    const addContainer = headerEl.createDiv({ cls: 'octo-mcp-add-container' });
    ...
    const dropdown = addContainer.createDiv({ cls: 'octo-mcp-add-dropdown' });

    const stdioOption = dropdown.createDiv({ cls: 'octo-mcp-add-option' });
    setIcon(stdioOption.createSpan({ cls: 'octo-mcp-add-option-icon' }), 'terminal');
    ...
    const httpOption = dropdown.createDiv({ cls: 'octo-mcp-add-option' });
    setIcon(httpOption.createSpan({ cls: 'octo-mcp-add-option-icon' }), 'globe');
    ...
    const importOption = dropdown.createDiv({ cls: 'octo-mcp-add-option' });
    setIcon(importOption.createSpan({ cls: 'octo-mcp-add-option-icon' }), 'clipboard-paste');
    ...
    if (this.servers.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'octo-mcp-empty' });
      emptyEl.setText('No mcp servers configured. Click "add" to add one.');
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'octo-mcp-list' });
  ...
  private renderServerItem(listEl: HTMLElement, server: ManagedMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'octo-mcp-item' });
    if (!server.enabled) {
      itemEl.addClass('octo-mcp-item-disabled');
    }

    const statusEl = itemEl.createDiv({ cls: 'octo-mcp-status' });
    statusEl.addClass(
      server.enabled ? 'octo-mcp-status-enabled' : 'octo-mcp-status-disabled'
    );

    const infoEl = itemEl.createDiv({ cls: 'octo-mcp-info' });

    const nameRow = infoEl.createDiv({ cls: 'octo-mcp-name-row' });

    const nameEl = nameRow.createSpan({ cls: 'octo-mcp-name' });
    nameEl.setText(server.name);

    const serverType = getMcpServerType(server.config);
    const typeEl = nameRow.createSpan({ cls: 'octo-mcp-type-badge' });
    typeEl.setText(serverType);

    if (server.contextSaving) {
      const csEl = nameRow.createSpan({ cls: 'octo-mcp-context-saving-badge' });
      csEl.setText('@');
      csEl.setAttribute('title', 'Context-saving: mention with @' + server.name + ' to enable');
    }

    const previewEl = infoEl.createDiv({ cls: 'octo-mcp-preview' });
    ...
    const actionsEl = itemEl.createDiv({ cls: 'octo-mcp-actions' });

    const testBtn = actionsEl.createEl('button', {
      cls: 'octo-mcp-action-btn',
    ...
    const deleteBtn = actionsEl.createEl('button', {
      cls: 'octo-mcp-action-btn octo-mcp-delete-btn',
