...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('octo-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'octo-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'octo-fork-target-option', text: label });
