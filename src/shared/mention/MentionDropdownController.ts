...
export class MentionDropdownController {
  ...
  constructor(
    ...
    this.dropdown = new SelectableDropdown<MentionItem>(this.containerEl, {
      listClassName: 'octo-mention-dropdown',
      itemClassName: 'octo-mention-item',
      emptyClassName: 'octo-mention-empty',
      fixed: this.fixed,
      fixedClassName: 'octo-mention-dropdown-fixed',
    });
  }
  ...
  private renderMentionDropdown(): void {
    this.dropdown.render({
      ...
      renderItem: (item, itemEl) => {
        const iconEl = itemEl.createSpan({ cls: 'octo-mention-icon' });
        switch (item.type) {
          case 'mcp-server':
        ...
        const textEl = itemEl.createSpan({ cls: 'octo-mention-text' });

        switch (item.type) {
          case 'mcp-server':
            textEl.createSpan({ cls: 'octo-mention-name' }).setText(`@${item.name}`);
            break;
          case 'agent-folder':
            textEl.createSpan({
              cls: 'octo-mention-name octo-mention-name-agent-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'agent': {
            // Show ID (which is namespaced for plugin agents) for consistency with inserted text
            textEl.createSpan({
              cls: 'octo-mention-name octo-mention-name-agent',
            }).setText(`@${item.id}`);
            if (item.description) {
              textEl.createSpan({ cls: 'octo-mention-agent-desc' }).setText(item.description);
            }
            break;
          }
          case 'context-folder':
            textEl.createSpan({
              cls: 'octo-mention-name octo-mention-name-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'context-file':
            textEl.createSpan({
              cls: 'octo-mention-name octo-mention-name-context',
            }).setText(item.name);
            break;
          case 'folder':
            textEl.createSpan({
            ...
            break;
          default:
            textEl.createSpan({ cls: 'octo-mention-path' }).setText(item.path || item.name);
        }
      },
  ...
  private positionFixed(): void {
    ...
    dropdownEl.setCssProps({
      '--octo-fixed-dropdown-bottom': `${window.innerHeight - inputRect.top + 4}px`,
      '--octo-fixed-dropdown-left': `${inputRect.left}px`,
      '--octo-fixed-dropdown-width': `${Math.max(inputRect.width, 280)}px`,
    });
  }
