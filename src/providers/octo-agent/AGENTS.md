...
- `OctoAgentTitleGenerationService` does not call the server; it truncates the user message client-side. Syncing the resulting title to the server session happens later, in `ConversationController`, via `OctoAgentChatRuntime.renameSession()`.
