...
// ============================================
// Fake task-result interpreter
//
// `SubagentManager` is provider-neutral and takes its
// `ProviderTaskResultInterpreter` by constructor injection, defaulting to
// `ProviderRegistry.getTaskResultInterpreter()` (the current default provider)
// when the caller doesn't supply one. These tests exercise `SubagentManager`'s
// own parsing/lifecycle logic plus the specific interpreter contract points it
// delegates to (agent-id extraction, structured-result extraction, terminal
// status resolution, async-launch detection, XML tag extraction) — they must
// not depend on whichever provider happens to be globally default. This fixture
// reproduces just the parsing rules the tests below actually assert, as a
// plain local mock of the `ProviderTaskResultInterpreter` shape.
// ============================================
...
function fixtureExtractDirectAgentId(toolUseResult: unknown): string | null {
  if (!isPlainRecord(toolUseResult)) return null;

  const direct = toolUseResult.agentId ?? toolUseResult.agent_id;
...
function fixtureExtractAgentIdFromString(value: string): string | null {
  const patterns = [
    /"agent_id"\s*:\s*"([^"]+)"/,
    /"agentId"\s*:\s*"([^"]+)"/,
...
function fixtureExtractFirstTextBlock(content: unknown): string | null {
  if (!Array.isArray(content)) return null;

  const firstTextBlock = content.find(
    (block) => isPlainRecord(block) && block.type === 'text' && typeof block.text === 'string',
  );
  if (!isPlainRecord(firstTextBlock) || typeof firstTextBlock.text !== 'string') {
    return null;
  }
...
function fixtureExtractResultFromTaskObject(task: unknown): string | null {
...
function fixtureExtractXmlTag(content: string, tagName: string): string | null {
...
function createFakeTaskResultInterpreter(): ProviderTaskResultInterpreter {
  return {
    hasAsyncLaunchMarker(toolUseResult: unknown): boolean {
      if (!isPlainRecord(toolUseResult)) return false;
      if (toolUseResult.isAsync === true) return true;
      ...
      return typeof toolUseResult.outputFile === 'string' && toolUseResult.outputFile.length > 0;
    },

    extractAgentId(toolUseResult: unknown): string | null {
      const directId = fixtureExtractDirectAgentId(toolUseResult);
      ...
      if (Array.isArray(toolUseResult.content)) {
        for (const block of toolUseResult.content) {
          if (typeof block === 'string') {
            const extracted = fixtureExtractAgentIdFromString(block);
            if (extracted) return extracted;
            continue;
          }
          if (!isPlainRecord(block) || typeof block.text !== 'string') continue;
          const extracted = fixtureExtractAgentIdFromString(block.text);
          if (extracted) return extracted;
        }
      }

      if (typeof toolUseResult.content === 'string') {
        return fixtureExtractAgentIdFromString(toolUseResult.content);
      }
    ...
    extractStructuredResult(toolUseResult: unknown): string | null {
      ...
      const taskResult = fixtureExtractResultFromTaskObject(toolUseResult.task);
      if (taskResult) return taskResult;

      const result = typeof toolUseResult.result === 'string' ? toolUseResult.result.trim() : '';
      if (result.length > 0) return result;

      const output = typeof toolUseResult.output === 'string' ? toolUseResult.output.trim() : '';
      if (output.length > 0) return output;

      return fixtureExtractFirstTextBlock(toolUseResult.content);
    },
    ...
    resolveTerminalStatus(
      ...
      if (!isPlainRecord(toolUseResult)) return fallbackStatus;
    ...
    extractTagValue(payload: string, tagName: string): string | null {
      return fixtureExtractXmlTag(payload, tagName);
    },
  };
}

const createManager = (interpreter: ProviderTaskResultInterpreter = createFakeTaskResultInterpreter()) => {
...
${inlineOutput}
    ...
    it('ignores truncated full output files outside trusted temp roots', () => {
      ...
      const untrustedDir = mkdtempSync(join(homeDir, '.octo-untrusted-'));
