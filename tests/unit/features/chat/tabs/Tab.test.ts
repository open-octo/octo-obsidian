...
jest.mock('@/providers/octo-agent/runtime/OctoAgentChatRuntime', () => ({
  OctoAgentChatRuntime: jest.fn().mockImplementation(() => ({
...
const createMockOctoAgentService = (overrides?: {
  ensureReady?: jest.Mock;
  syncConversationState?: jest.Mock;
  onReadyStateChange?: jest.Mock;
  providerId?: string;
}) => ({
  providerId: overrides?.providerId ?? 'octo-agent',
...
function createMockPlugin(overrides: Record<string, any> = {}): any {
  ...
  return {
    ...
    agentManager: agentMentionProvider,
...
describe('Tab - Creation', () => {
  describe('createTab', () => {
    ...
    it('should fall back to settings.model when no defaultProviderId is given', () => {
      ...
      plugin.settings.model = OCTO_AGENT_MODEL;

      const tab = createTab(createMockOptions({ plugin }));

      expect(tab.lifecycleState).toBe('blank');
      expect(tab.draftModel).toBe(OCTO_AGENT_MODEL);
...
describe('Tab - Service Initialization', () => {
  ...
  describe('initializeTabService', () => {
    it('should not reinitialize if already initialized', async () => {
      ...
      tab.service = createMockOctoAgentService() as any;

      await initializeTabService(tab, options.plugin, options.mcpManager);

      // Service should not be replaced
      expect(tab.service).toEqual(expect.objectContaining({ providerId: 'octo-agent' }));
    });

    it('should create the runtime on first initialization', async () => {
      const options = createMockOptions();
      const tab = createTab(options);
    ...
    it('should NOT call ensureReady for blank tabs (lazy start)', async () => {
      ...
      const runtimeModule = jest.requireMock('@/providers/octo-agent/runtime/OctoAgentChatRuntime') as { OctoAgentChatRuntime: jest.Mock };
      runtimeModule.OctoAgentChatRuntime.mockImplementationOnce(() => createMockOctoAgentService({ ensureReady: mockEnsureReady }));
    ...
    it('should sync existing conversations with saved external contexts', async () => {
      ...
      const runtimeModule = jest.requireMock('@/providers/octo-agent/runtime/OctoAgentChatRuntime') as { OctoAgentChatRuntime: jest.Mock };
      runtimeModule.OctoAgentChatRuntime.mockImplementationOnce(() => createMockOctoAgentService({
    ...
    it('should initialize toolbar config for the tab provider', () => {
      ...
      expect(getChatUIConfigSpy).toHaveBeenCalledWith('octo-agent');
      expect(getCapabilitiesSpy).toHaveBeenCalledWith('octo-agent');
    });
    ...
    it('resolves the agent mention service through the provider-specific lookup', () => {
      ...
      const getAgentMentionProviderSpy = jest.spyOn(ProviderWorkspaceRegistry, 'getAgentMentionProvider')
        .mockReturnValue(agentMentionProvider as any);
      const plugin = createMockPlugin();
      const tab = createTab(createMockOptions({
        plugin,
        conversation: {
          id: 'conv-octo-agent-mention',
          providerId: 'octo-agent',
          title: 'Octo Agent mention split',
          messages: [],
          sessionId: null,
          createdAt: Date.now(),
          updatedAt: Date.now(),
      ...
      expect(getAgentMentionProviderSpy).toHaveBeenCalledWith('octo-agent');
      expect(mockFileContextManager.setAgentService).toHaveBeenCalledWith(agentMentionProvider);
    });
    ...
    it('resets to blank state when the new-conversation callback fires', () => {
      ...
      expect(tab.draftModel).toBe(plugin.settings.savedProviderModel['octo-agent']);
      expect(tab.serviceInitialized).toBe(false);
    });
    ...
    it('cleans up the active runtime when resetting to a new blank session', () => {
      ...
      const plugin = createMockPlugin();
      plugin.settings.savedProviderModel = { 'octo-agent': OCTO_AGENT_MODEL };
      const tab = createTab(createMockOptions({ plugin }));
      initializeTabUI(tab, plugin);
      initializeTabControllers(tab, plugin, {} as any, createMockMcpManager());

      const staleService = createMockOctoAgentService();
...
describe('Tab - Destruction', () => {
  describe('destroyTab', () => {
    ...
    it('should unsubscribe from ready state changes when tab is destroyed', async () => {
      ...
      const runtimeModule = jest.requireMock('@/providers/octo-agent/runtime/OctoAgentChatRuntime') as { OctoAgentChatRuntime: jest.Mock };
      runtimeModule.OctoAgentChatRuntime.mockImplementationOnce(() => createMockOctoAgentService({ onReadyStateChange: mockOnReadyStateChange }));
...
describe('Tab - UI Initialization', () => {
  ...
  describe('initializeTabUI', () => {
    ...
    it('should create bang-bash mode from provider UI config', () => {
      ...
      const getChatUIConfigSpy = jest.spyOn(ProviderRegistry, 'getChatUIConfig').mockReturnValue({
        getModelOptions: jest.fn().mockReturnValue([]),
        ownsModel: jest.fn().mockReturnValue(false),
        isAdaptiveReasoningModel: jest.fn().mockReturnValue(false),
        getReasoningOptions: jest.fn().mockReturnValue([]),
        ...
        normalizeModelVariant: jest.fn((model: string) => model),
        getCustomModelIds: jest.fn().mockReturnValue(new Set()),
        isBangBashEnabled: jest.fn().mockReturnValue(true),
      } as any);
      const plugin = createMockPlugin();
      const options = createMockOptions({ plugin });
      const tab = createTab(options);
...
describe('Tab - UI Callback Wiring', () => {
  ...
  describe('initializeTabUI callbacks', () => {
    ...
    it('should wire provider catalog config when provided in options', async () => {
      ...
      const mockConfig = { providerId: 'octo-agent' as const, triggerChars: ['/'], builtInPrefix: '/', skillPrefix: '/', commandPrefix: '/' };
      const plugin = createMockPlugin();
      const options = createMockOptions({ plugin });
      const tab = createTab(options);
...
describe('Tab - Service Initialization Error Handling', () => {
  ...
  it('should skip re-initialization if already initialized', async () => {
    ...
    tab.serviceInitialized = true;
    const originalService = createMockOctoAgentService() as any;
...
describe('Tab - handleForkRequest', () => {
  ...
  it('should fall back to conversation session ID when service has none', async () => {
    const plugin = createMockPlugin({
      getConversationSync: jest.fn().mockReturnValue({
        providerState: { sessionId: 'conv-session-xyz' },
        title: 'Fallback Chat',
      }),
    });
    const { tab, forkCallback, forkRequestCallback } = setupForkTest({ plugin });
  ...
  it('should prefer service session ID over conversation metadata', async () => {
    const plugin = createMockPlugin({
      getConversationSync: jest.fn().mockReturnValue({
        title: 'Test',
        providerState: { sessionId: 'conv-session' },
...
describe('Tab - Blank Tab Model Selector', () => {
  ...
  it('returns the enabled provider models grouped by display name', () => {
    const octoAgentModels = [
      { value: OCTO_AGENT_MODEL, label: 'Octo Agent' },
    ];
    jest.spyOn(ProviderRegistry, 'getEnabledProviderIds').mockReturnValue(['octo-agent']);
    jest.spyOn(ProviderRegistry, 'getProviderDisplayName').mockReturnValue('Octo Agent');
    jest.spyOn(ProviderRegistry, 'getChatUIConfig').mockImplementation(() => ({
      getModelOptions: () => octoAgentModels,
...
describe('Tab - First Send Binding', () => {
  it('derives provider from draft model on first send', async () => {
    ...
    runtimeModule.OctoAgentChatRuntime.mockImplementationOnce(() => createMockOctoAgentService({ ensureReady: mockEnsureReady }));
    const createChatRuntimeSpy = jest.spyOn(ProviderRegistry, 'createChatRuntime')
      .mockReturnValue(createMockOctoAgentService() as any);

    const plugin = createMockPlugin();
    const tab = createTab(createMockOptions({ plugin }));

    tab.draftModel = OCTO_AGENT_MODEL;
...
describe('Tab - History Bind Without Runtime', () => {
  ...
  it('ensureServiceForConversation wires the provider catalog and hidden commands', async () => {
    ...
    initializeTabUI(tab, plugin, {
      getProviderCatalogConfig: () => ({
        config: octoAgentCatalog.getDropdownConfig(),
    ...
    expect(octoAgentCatalog.listDropdownEntries).not.toHaveBeenCalled();
  });
});
...
describe('Tab - InputController getTabProviderId wiring', () => {
  it('wires getTabProviderId to InputController deps', () => {
    ...
    // For a blank tab with default model, should resolve to octo-agent
    const result = config.getTabProviderId();
    expect(result).toBe('octo-agent');
  });
});
