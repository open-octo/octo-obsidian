...
const OCTO_AGENT_MODEL = 'octo-agent/kimi-for-coding';
...
jest.mock('@/core/providers/ProviderRegistry', () => ({
  ProviderRegistry: {
    ...
    resolveProviderForModel: () => 'octo-agent',
  },
}));
  ...
  describe('getPersistedState', () => {
    ...
    it('should persist draftModel for blank tabs', async () => {
      ...
      expect(blankManager.getPersistedState()).toEqual({
        activeTabId: 'blank-octo-agent',
        openTabs: [{
          tabId: 'blank-octo-agent',
          conversationId: null,
          draftModel: OCTO_AGENT_MODEL,
        }],
      });
...
describe('TabManager - SDK Commands', () => {
  ...
  it('should reuse commands from another ready tab with the same provider', async () => {
    ...
    await manager.createTab();
    const lazyTab = await manager.createTab();

    await expect(manager.getSdkCommands(lazyTab!.id)).resolves.toEqual(supportedCommands);
    expect(readyService.getSupportedCommands).toHaveBeenCalledTimes(1);
  });

  it('should prime active blank octo-agent tabs automatically', async () => {
    ...
    mockGetCapabilities.mockImplementation((providerId: string) => ({
      ...
      supportsPersistentRuntime: true,
      supportsNativeHistory: true,
      supportsPlanMode: providerId === 'octo-agent',
      supportsRewind: providerId === 'octo-agent',
      supportsFork: providerId === 'octo-agent',
      supportsProviderCommands: providerId === 'octo-agent',
      reasoningControl: providerId === 'octo-agent' ? 'effort' : 'none',
    }));
    const manager = createManager({
      plugin: createMockPlugin(),
      tabFactory: () => createMockTabData({
  ...
  it('should prime the active restored octo-agent conversation tab automatically', async () => {
    const supportedCommands = [{ id: 'acp:review', name: 'review', content: '' }];
    const mockCatalog = {
      setRuntimeCommands: jest.fn(),
    };
    ...
    const plugin = createMockPlugin({
      getConversationById: jest.fn().mockResolvedValue({
        id: 'conv-octo-agent',
        messages: [{ id: 'm1' }],
        providerState: { databasePath: '/persisted/octo-agent.db' },
        sessionId: 'session-1',
      }),
    });
    const manager = createManager({
      plugin,
      tabFactory: () => createMockTabData({
        id: 'tab-octo-agent-restored',
        providerId: 'octo-agent',
        conversationId: 'conv-octo-agent',
    ...
    await manager.createTab('conv-octo-agent', 'tab-octo-agent-restored', { activate: false });
    await flushMicrotasks();
  ...
  it('should prime the active restored pre-session octo-agent conversation tab automatically', async () => {
    const supportedCommands = [{ id: 'acp:review', name: 'review', content: '' }];
    const mockCatalog = {
      setRuntimeCommands: jest.fn(),
    };
    ...
    await manager.createTab('conv-octo-agent', 'tab-octo-agent-pre-session', { activate: false });
    await flushMicrotasks();
  ...
  it('should not borrow ready octo-agent commands from another tab session', async () => {
    ...
    const manager = createManager({
      ...
      tabFactory: (n) => createMockTabData({
        id: `tab-${n}`,
        providerId: 'octo-agent',
        conversationId: n === 2 ? 'conv-octo-agent' : null,
    ...
    const coldTab = await manager.createTab('conv-octo-agent');
  ...
  it('should keep an active restored octo-agent conversation tab cold', async () => {
    ProviderWorkspaceRegistry.setServices('octo-agent', {
      commandCatalog: {
        setRuntimeCommands: jest.fn(),
      } as any,
    ...
    const tab = await manager.createTab('conv-octo-agent', 'tab-octo-agent-restored', { activate: false });
    await flushMicrotasks();
...
describe('TabManager - Provider Command Catalog', () => {
  const mockCatalogEntries = [
    {
      id: 'octo-agent-skill-analyze', providerId: 'octo-agent', kind: 'skill',
  ...
  afterEach(() => {
    ProviderWorkspaceRegistry.setServices('octo-agent', undefined);
  });

  it('should pass provider catalog config to initializeTabUI for octo-agent tab', async () => {
    ProviderWorkspaceRegistry.setServices('octo-agent', { commandCatalog: mockCatalog as any });

    const manager = createManager({
      tabFactory: () => createMockTabData({ id: 'tab-1', providerId: 'octo-agent' }),
    });
  ...
  it('should provide scan-backed entries for octo-agent without runtime', async () => {
    ProviderWorkspaceRegistry.setServices('octo-agent', { commandCatalog: mockCatalog as any });
  ...
  it('should refresh octo-agent runtime commands before listing catalog entries', async () => {
    ...
    ProviderWorkspaceRegistry.setServices('octo-agent', { commandCatalog: octoAgentCatalog as any });
    ...
    expect(readyService.getSupportedCommands).toHaveBeenCalledTimes(1);
    expect(octoAgentCatalog.setRuntimeCommands).toHaveBeenCalledWith(supportedCommands);
    expect(octoAgentCatalog.listDropdownEntries).toHaveBeenCalledWith({ includeBuiltIns: false });
  });

  it('should clear octo-agent runtime commands when revalidation returns no commands', async () => {
    const readyService = {
      providerId: 'octo-agent',
      isReady: jest.fn().mockReturnValue(true),
    ...
    expect(octoAgentCatalog.setRuntimeCommands).toHaveBeenCalledWith([]);
  });
  ...
  it('starts blank-tab provider warmup in the background from the provider-change callback', async () => {
    ...
    let settled = false;
    const callbackPromise = Promise.resolve(options.onProviderChanged('octo-agent')).then(() => {
      settled = true;
    });
  ...
  it('should return null catalog config when provider has no catalog', async () => {
    // No catalog assigned to registry for 'octo-agent'
...
describe('TabManager - forkToNewTab', () => {
  it('should propagate currentNote from context to forked conversation', async () => {
    const mockCreateConversation = jest.fn().mockResolvedValue({ id: 'fork-conv-1', providerId: 'octo-agent' });
  ...
  it('should not set currentNote when context has none', async () => {
    const mockCreateConversation = jest.fn().mockResolvedValue({ id: 'fork-conv-2', providerId: 'octo-agent' });
    const mockUpdateConversation = jest.fn().mockResolvedValue(undefined);
...
describe('TabManager - forkInCurrentTab', () => {
  ...
  it('should return false when no active tab exists', async () => {
    const plugin = createMockPlugin({
      createConversation: jest.fn().mockResolvedValue({ id: 'fork-conv-2', providerId: 'octo-agent' }),
  ...
  it('should not check tab count limit', async () => {
    const mockCreateConversation = jest.fn().mockResolvedValue({ id: 'fork-conv-3', providerId: 'octo-agent' });
    const mockUpdateConversation = jest.fn().mockResolvedValue(undefined);
    const mockSwitchTo = jest.fn().mockResolvedValue(undefined);
...
describe('TabManager - forkToNewTab at max tabs', () => {
  it('should return null when at max tabs', async () => {
    ...
    plugin.createConversation = jest.fn().mockResolvedValue({ id: 'fork-conv', providerId: 'octo-agent' });
...
describe('TabManager - createForkConversation', () => {
  ...
  it('should create the fork conversation with the source provider', async () => {
    const mockCreateConversation = jest.fn().mockResolvedValue({ id: 'fork-conv-octo-agent', providerId: 'octo-agent' });
    const mockUpdateConversation = jest.fn().mockResolvedValue(undefined);
    ...
    await manager.forkToNewTab({
      messages: [],
      providerId: 'octo-agent',
      sourceSessionId: 'session-octo-agent',
      resumeAt: 'asst-octo-agent',
    });

    expect(mockCreateConversation).toHaveBeenCalledWith({ providerId: 'octo-agent' });
  });
...
describe('TabManager - buildForkTitle', () => {
  function setupTitleTest(existingTitles: string[] = []) {
    const mockCreateConversation = jest.fn().mockResolvedValue({ id: 'fork-conv', providerId: 'octo-agent' });
    const mockUpdateConversation = jest.fn().mockResolvedValue(undefined);
