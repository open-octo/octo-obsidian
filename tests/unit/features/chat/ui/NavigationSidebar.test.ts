...
describe('NavigationSidebar', () => {
  ...
  describe('initialization', () => {
    it('should create container with correct class', () => {
      ...
      const container = parentEl.querySelector('.octo-nav-sidebar');
  ...
  describe('previous/next message navigation', () => {
    function addUserMessage(el: MockElement, offset: number): MockElement {
      const msg = el.createDiv({ cls: 'octo-message octo-message-user' });
      msg.offsetTop = offset;
      return msg;
    }

    function addAssistantMessage(el: MockElement, offset: number): MockElement {
      const msg = el.createDiv({ cls: 'octo-message octo-message-assistant' });
      msg.offsetTop = offset;
      return msg;
    }
    ...
    function getButtons(parent: MockElement) {
      const container = parent.querySelector('.octo-nav-sidebar')!;
      return {
  ...
  describe('conversation directory', () => {
    function addMessage(
      ...
      const msg = el.createDiv({ cls: `octo-message octo-message-${role}` });
      msg.offsetTop = offset;
      if (tocTitle) {
    ...
    it('should show an empty directory state when there are no user messages', () => {
      ...
      expect(directoryBtn.classList.contains('octo-hidden')).toBe(false);

      directoryBtn.click();

      const popover = parentEl.querySelector('.octo-nav-toc-popover');
      const emptyState = parentEl.querySelector('.octo-nav-toc-empty');
    ...
    it('should render directory entries for user messages only', () => {
      ...
      const popover = parentEl.querySelector('.octo-nav-toc-popover');
      const items = parentEl.querySelectorAll('.octo-nav-toc-item');
      expect(popover).not.toBeNull();
    ...
    it('should scroll to a selected directory entry and close the directory', () => {
      ...
      expect(lastCall.behavior).toBe('smooth');
      expect(parentEl.querySelector('.octo-nav-toc-popover')).toBeNull();
    });
    ...
    it('should close the directory when the directory button is clicked again', () => {
      ...
      const directoryBtn = getDirectoryButton(parentEl);
      directoryBtn.click();
      expect(parentEl.querySelector('.octo-nav-toc-popover')).not.toBeNull();
    ...
    it('should not rebuild an open directory for assistant content mutations', () => {
      ...
      const assistantContent = assistantMsg.createDiv({ cls: 'octo-message-content' });
      ...
      const directoryBtn = getDirectoryButton(parentEl);
      directoryBtn.click();
      const originalPopover = parentEl.querySelector('.octo-nav-toc-popover');
      ...
      expect(parentEl.querySelector('.octo-nav-toc-popover')).toBe(originalPopover);
    });
    ...
    it('should refresh an open directory when a user message toc title changes', () => {
      ...
      const updatedPopover = parentEl.querySelector('.octo-nav-toc-popover');
      const items = parentEl.querySelectorAll('.octo-nav-toc-item');
  ...
  describe('destroy', () => {
    it('should remove container from DOM', () => {
      ...
      expect(parentEl.querySelector('.octo-nav-sidebar')).not.toBeNull();

      sidebar.destroy();

      expect(parentEl.querySelector('.octo-nav-sidebar')).toBeNull();
    });
  });
});
