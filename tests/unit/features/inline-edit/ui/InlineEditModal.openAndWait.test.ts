...
describe('InlineEditModal - openAndWait', () => {
  ...
  it('uses provider-scoped hidden commands for inline edit dropdowns', async () => {
    const originalDocument = (global as any).document;
    (global as any).document = {
      body: createMockEl('body'),
      createElement: (tagName: string) => createMockEl(tagName),
    ...
    try {
      ...
      const plugin = {
        ...
        getView: jest.fn().mockReturnValue({
          getActiveTab: jest.fn().mockReturnValue({
            conversationId: null,
            draftModel: 'octo-agent/custom-model',
            providerId: 'octo-agent',
            service: {
              getAuxiliaryModel: jest.fn().mockReturnValue('octo-agent/custom-model'),
              providerId: 'octo-agent',
            },
          }),
        }),
      ...
      expect(providerSpy).toHaveBeenCalledWith(plugin, 'octo-agent');
      expect(inlineEditService.setModelOverride).toHaveBeenCalledWith('octo-agent/custom-model');
  ...
  it('renders accept and reject controls in the block preview', async () => {
    ...
    try {
      ...
      const actionBar = previewEl.querySelector('.octo-inline-preview-actions');
      const actionButtons = previewEl.querySelectorAll('.octo-inline-preview-action');
  ...
  it('renders markdown diff documents with block context', async () => {
    ...
    try {
      ...
      const diffBlocks = previewEl.querySelectorAll('.octo-diff-block');
      expect(diffBlocks).toHaveLength(2);
      expect(diffBlocks[0].hasClass('octo-diff-del')).toBe(true);
      expect(diffBlocks[1].hasClass('octo-diff-ins')).toBe(true);
