...
describe('createProviderIconSvg', () => {
  it('renders path-based provider icons with currentColor fill', () => {
    ...
    expect(svg.classList.contains('octo-provider-icon')).toBe(true);
  ...
  it('renders composite provider icons with theme variants', () => {
    ...
    expect(svg.querySelector('.octo-provider-icon-variant--light')).not.toBeNull();
    expect(svg.querySelector('.octo-provider-icon-variant--dark')).not.toBeNull();
  });
